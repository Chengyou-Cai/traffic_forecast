...
def generate_train_valid_test(fdir='_metr_la',fname='metr-la.h5',seq_x_len=12,seq_y_len=12):
    df = pd.read_hdf(os.path.join(fdir, fname))
    # 预测窗口 1 hour = 12 * 5 min
    x_offsets = np.sort(np.arange(-(seq_x_len-1), 1, 1))
    y_offsets = np.sort(np.arange(1, (seq_y_len+1), 1))
